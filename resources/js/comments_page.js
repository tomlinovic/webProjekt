...
function renderComments() {
    ...
    list.forEach(c => {
        ...
        div.innerHTML = `
            ...
            <div class="comment-text" id="comment-text-${commentId}">${c.text}</div>
            <div class="comment-actions">
                ${c.user.id === userId
                    ? `<button class="edit-btn" data-id="${commentId}">Edit</button>
                       <button class="delete-btn" data-id="${commentId}">Delete</button>`
                    : `<button class="report-btn" data-id="${commentId}">Report</button>`
                }
            </div>
        `;
...
document.addEventListener("click", (e) => {
    ...
    const oldText = textDiv.innerText.trim();
    ...
    try {
        ...
        if (data.success) {
            const updateList = (list) => {
                const item = list.find(c => parseInt(c.id) === id);
...
document.addEventListener("click", (e) => {
    ...
    const original = allComments.find(c => parseInt(c.id) === id)?.text;
    const textDiv = document.getElementById(`comment-text-${id}`);
    if (textDiv && original) textDiv.innerHTML = original;
});
...
document.addEventListener("click", async (e) => {
    ...
    if (data.success) {
        const removeFrom = (list) => {
            const index = list.findIndex(c => parseInt(c.id) === id);
